...
import {
  ...
  Receipt, ShoppingCart, Calculator, Lock, MessageSquare
} from 'lucide-react'
...
const NAV_ITEMS = [
  ...
  { key:'messages',     label:'Messages',        href:'/dashboard/messages',     icon:MessageSquare,   module:null },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: profileData } = await supabase
        .from('profiles').select('*, organizations(*)').eq('id', session.user.id).single()
      if (profileData) {
  ...
  useEffect(() => {
    ...
    const checkMsgs = async () => {
      const { count } = await supabase.from('app_messages')
  ...
  // Module gating — accounting always free, dashboard/messages/settings always visible
  const enabledModules: string[] = (organization?.settings as any)?.enabled_modules || ['accounting']
  const visibleItems = NAV_ITEMS.filter(item =>
    item.module === null ||
    item.module === 'accounting' ||
      ...
      <aside className={cn(
        ...
        <div className={cn('flex items-center gap-3 px-4 py-5 border-b border-slate-100', !sidebarOpen&&'justify-center px-2')}>
          ...
          {sidebarOpen && (
            <div className="min-w-0">
              ...
              {organization?.name && <p className="text-xs text-slate-400 truncate max-w-36">{organization.name}</p>}
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
          {visibleItems.map(item => {
            ...
            const isMsgs  = item.key === 'messages'
            return (
              <Link key={item.key} href={item.href}
                ...
                {sidebarOpen && isMsgs && unreadMsgs > 0 && (
                  <span className="ml-auto badge text-xs" style={{ background:'var(--danger)', color:'white' }}>
                    {unreadMsgs}
                  </span>
                )}
              </Link>
          ...
          {sidebarOpen && enabledModules.length <= 1 && (
            <div className="mt-3 mx-1 p-3 rounded-xl bg-amber-50 border border-amber-200">
              ...
              <p className="text-xs text-amber-600 leading-relaxed">
                Upgrade in Settings to unlock Transactions, Payroll, POS and more.
              </p>
            </div>
          )}
        </nav>
        ...
        <div className={cn('p-3 border-t border-slate-100', !sidebarOpen&&'flex flex-col items-center gap-2')}>
          {sidebarOpen ? (
            <div className="flex items-center gap-3 px-2 py-2">
              ...
              <button onClick={handleSignOut} className="ml-auto text-slate-400 hover:text-red-500 transition-colors" title="Sign out">
                <LogOut size={15}/>
              </button>
            </div>
          ) : (
        ...
        <header className="h-14 bg-white border-b border-slate-200 flex items-center px-6 gap-4 flex-shrink-0">
          <div className="flex-1 max-w-md">
            <div className="relative">
              ...
              <input className="w-full pl-9 pr-4 py-2 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent"
