...
import {
  ...
  AlertCircle, X, AlertTriangle, ChevronDown, UserPlus
} from 'lucide-react'
...
// Shows matching contacts as you type; lets you create a new one on the fly.
function ContactAutocomplete({
  value,
  onChange,
  contacts,
  onCreateContact,
  placeholder = 'Narration / contact name…',
}: {
  value: string
  ...
  const exactMatch = contacts.some(c => c.name.toLowerCase() === value.toLowerCase())
  const showCreate = value.trim().length > 1 && !exactMatch
        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
          {filtered.slice(0, 6).map(c => (
            <button
              key={c.id}
              type="button"
              className="w-full text-left px-3 py-2 text-xs hover:bg-brand-50 flex items-center gap-2"
              onMouseDown={() => { onChange(c.name); setOpen(false) }}
            >
              <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                c.type === 'customer' ? 'bg-green-500' :
                c.type === 'supplier' ? 'bg-amber-500' : 'bg-brand-500'
              }`} />
              <span className="font-medium text-slate-800">{c.name}</span>
              <span className="text-slate-400 ml-auto capitalize">{c.type}</span>
            </button>
          ))}
          {showCreate && (
            <button
              type="button"
              disabled={creating}
              className="w-full text-left px-3 py-2 text-xs hover:bg-green-50 flex items-center gap-2 border-t border-slate-100 text-green-700"
              onMouseDown={async () => {
                setCreating(true)
                await onCreateContact(value.trim())
                setCreating(false)
                setOpen(false)
              }}
            >
              <UserPlus size={11} />
              {creating ? 'Creating…' : `Create contact "${value.trim()}"`}
            </button>
          )}
        </div>
      )}
...
export default function AccountingPage() {
  ...
  // Create a contact on the fly when user types a new name
  const handleCreateContact = async (name: string) => {
    ...
    if (error) { toast.error('Could not create contact'); return }
  ...
  const getAccountCredit = (a: Account): number => {
    ...
    if (a.account_type?.normal_balance === 'debit' && a.balance < 0) return Math.abs(a.balance)
    return 0
  }
            ...
            <table className="table">
              <thead>
                <tr>
                  ...
                  <th>Type</th><th className="text-right">Debit</th>
                  <th className="text-right">Credit</th><th>Status</th><th></th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr>
                    ...
                    .map(entry => (
                      <tr key={entry.id}>
                        ...
                        <td>
                          <button className="btn-ghost p-1.5" onClick={() => setViewEntry(entry)}><Eye size={14} /></button>
                        </td>
                      </tr>
                    ))}
              </tbody>
      ...
      {tab === 1 && (
        <div className="card overflow-hidden">
          <div className="table-container">
            <table className="table">
              <thead>
                <tr><th>Code</th><th>Account Name</th><th>Category</th><th>Normal Balance</th><th className="text-right">Balance</th><th>Status</th></tr>
              </thead>
              <tbody>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-12 gap-2 mb-1 px-1">
                  ...
                  <div className="col-span-4 text-xs text-slate-400 font-medium">Narration / Contact</div>
                      ...
                      <div className="col-span-4">
                        <select className="input text-xs" value={line.account_id}
                          ...
                          {CATEGORY_ORDER.map(cat => {
                            const catAccounts = accounts.filter(a => a.account_type?.category === cat)
                ...
                <div className="grid grid-cols-12 gap-2 mt-3 pt-3 border-t border-slate-200">
                  <div className="col-span-8 text-xs font-semibold text-slate-600">Totals</div>
              ...
              <div className="flex items-start gap-2 p-3 bg-brand-50 rounded-xl">
                <UserPlus size={14} className="text-brand-600 mt-0.5 flex-shrink-0" />
                <p className="text-xs text-brand-700">
                  <strong>Tip:</strong> Type a contact name (e.g. "Kamau", "Wafula") in the Narration column.
                  Existing contacts appear instantly; unknown names can be created on the fly.
                </p>
              </div>
            </div>
